...
videosModel.getOne = function(id){
	...
	if(!id){
		results.reject({status:'error', error:'Video Id not supplied.', code: 'VideoIdMissing'});
	}
	...
	Video.findOne({_id:id},function(err, dbVideo) {
		...
		if(dbVideo){
			...
			results.reject({status:'error', error:'Invalid video Id supplied.', code: 'InvalidVideoId'});
		}
...
videosModel.rate = function(id, ratting){
	...
	if(!ratting){
		results.reject({status:'error', error:'User ratting not supplied.', code: 'RatingMissing'});
		error = true;
	}

	if(ratting<0 || ratting>5){
		results.reject({status:'error', error:'User ratting is out of range.', code: 'RatingOutOfRange'});
		error = true;
	}
